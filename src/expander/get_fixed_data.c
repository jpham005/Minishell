...
/*   Updated: 2022/03/28 22:47:48 by jaham            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	is_valid_asterisk(char c, t_expanded_list *expanded_list, size_t i, t_quote_mask *mask)
{
	if (c != '*')
		return (0);
	while (expanded_list)
	{
		if (is_in_range_index(expanded_list->start, expanded_list->end, i) || check_quote_mask(*mask))
			return (0);
...
static int	is_valid_quote(char c, t_expanded_list *expanded_list, size_t i)
{
	if ((c != '\'') && (c != '\"'))
		return (0);
	while (expanded_list)
...
static char	*get_fixed_string(t_token *token, t_buffer *buffer, size_t *i)
{
			...
			&& !is_valid_asterisk(token->data[*i], token->expanded_list, *i, &mask))
	{
		...
		if (!is_valid_quote(token->data[*i], token->expanded_list, *i))
